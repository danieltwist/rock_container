@if($task->name != '')
    <a class="text-dark" href="{{ route('work_request.show', $task->id) }}">{{ $task->name }}</a>
@endif
...
@if(mb_strlen($task->text)>120)
    <div id="collapse_task_text_compact_{{ $task->id }}">
        ...
        <a class="cursor-pointer collapse-trigger" data-div_id="collapse_task_text_full_{{ $task->id }}"><i class="fa fa-angle-down"></i> {{ __('general.expand') }}</a>
    </div>
    <div id="collapse_task_text_full_{{ $task->id }}" class="d-none">
        {{ $task->text }}
        <a class="cursor-pointer collapse-trigger" data-div_id="collapse_task_text_compact_{{ $task->id }}"><i class="fa fa-angle-up"></i> {{ __('general.collapse') }}</a>
    </div>
@else
    {{ $task->text }}
@endif
