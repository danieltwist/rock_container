...
<div class="row invoice-info">
        ...
        @if ($invoice->agree_1 != '' || $invoice->agree_2 != '')
            ...
            @if ($invoice->agree_2 != '')
                @if(checkUploadedFileInvoice($invoice->id)['agree2'])
                    Чжао: @include('invoice.status_switch', ['status' => unserialize($invoice->agree_2)['status']]) {{ unserialize($invoice->agree_2)['date'] }}
                @else
                    Чжао: {{ $invoice->agree_2 }}
                @endif
            @endif
        @endif
    ...
    @endif
    @can ('agree invoices')
        @if(in_array($current_user_id, ['1','8','9']))
            @if ($invoice->status!='Оплачен' && $invoice->direction == 'Расход')
                <div class="col-md-12 mt-4">
                    <h5>{{ __('invoice.agree_invoice') }}</h5>
                    <form action="{{ route('agree_invoice_ntc') }}" method="POST">
                        @csrf
                        <input type="hidden" name="invoice_id" value="{{ $invoice->id }}">
                        <div class="form-group">
