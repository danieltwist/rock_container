@if ($container->usage_statistic->isNotEmpty())
    {{ __('container.snp') }}: {{ $container->usage_statistic[0]->snp_days_for_client }} {{ __('container.days') }} ({{ $container->usage_statistic[0]->snp_total_amount_for_client }}{{ $container->usage_statistic[0]->snp_currency }})
    / {{ $container->usage_statistic[0]->snp_days_for_us }} {{ __('container.days') }} ({{ $container->usage_statistic[0]->snp_total_amount_for_us }}{{ $container->usage_statistic[0]->snp_currency }})
@else
    @if ($container->start_date_for_us != '' || $container->start_date_for_client != '')
        <small>{{ __('container.end_grace_period') }}: {{ $container->usage_dates['end_grace_date']}} / {{ $container->usage_dates['end_grace_date_for_us']}}
            @if($container->usage_dates['overdue_days'] != 0 || $container->usage_dates['overdue_days_for_us'] != 0)
                <br>
                <b>{{ __('container.overdue_days') }}: {{ $container->usage_dates['overdue_days'] }} ({{ $container->usage_dates['snp_amount_for_client'] }}{{ $container->usage_dates['snp_currency'] }})
                    / {{ $container->usage_dates['overdue_days_for_us'] }} ({{ $container->usage_dates['snp_amount_for_us'] }}{{ $container->usage_dates['snp_currency'] }})
                </b>
            @endif
        </small>
    @endif
