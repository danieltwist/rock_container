...
<div class="timeline-item">
    ...
    @if($container_project->contract_with_terminal == '')
        <div class="timeline-body">
            <form action="{{ route('container_project.update', $container_project->id) }}" method="post" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <input type="hidden" name="action" value="upload_contract_with_terminal">
                <div class="form-group">
                    <input type="file" class="form-control-file" name="contract_with_terminal">
                </div>
                <button type="submit" class="btn btn-primary btn-sm"
                        data-action='{"update_div":{"div_id":"contract_with_terminal"}}'>
                    {{ __('general.upload') }}
                </button>
            </form>
        </div>
    @else
        <div class="timeline-footer">
            <form class="form-inline" action="{{ route('container_project.destroy', $container_project->id) }}" method="post">
                <a class="btn btn-outline-primary btn-sm" href="{{ Storage::url($container_project->contract_with_terminal) }}" download>Скачать</a>
                @csrf
                @method('DELETE')
                <input type="hidden" name="action" value="delete_contract_with_terminal">
