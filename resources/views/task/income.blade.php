...
@section('content')
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-12">
                    <h1 class="m-0">{{ __('task.my_tasks') }}</h1>
                </div>
            </div>
        </div>
    </div>
    <section class="content">
        <div class="container-fluid">
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="card">
                        <div class="card-header cursor-pointer" data-card-widget="collapse">
                            <h3 class="card-title">{{ __('task.accepted_tasks') }}</h3>
                        ...
                        <div class="card-body card-comments">
                            @if($accepted_tasks->isNotEmpty())
                                @foreach($accepted_tasks as $task)
                                    ...
                                    @endphp
                                        <div class="card-comment">
                                            ...
                                            @if($task->model != 'free')
                                                @if($task->model == 'upd')
                                                    {{ __('task.object') }}: <a href="{{ route('task.show', $task->id) }}">
                                                        {{ $task->object }}
                                                    </a><br>
                                                @elseif($task->model == 'container_project')
                                                    {{ __('task.object') }}: <a href="{{ route('container_project.index').'?need_to_process' }}">
                                                        {{ $task->object }}
                                                    </a><br>
                                                @else
                                                    {{ __('task.object') }}: <a href="{{ route($model.'.show',$task->model_id) }}">
                                                        {{ $task->object }}
                                                    </a><br>
                                                @endif
                                            @endif
                                                ...
                                                <br>
                                                {{ __('task.current_status') }}: @include('task.status_switch', ['status' => $task->status])<br>
                                                @if($task->info != '')
                                                    {{ __('task.Redirected') }}: {{ $task->info }} <br>
                                                @endif
                                                @if($task->deadline != '')
                                                    @if($task->overdue)
                                                        <strong class="text-danger">{{ __('task.finish_before') }}: {{ $task->deadline }}</strong>
                                                    @else
                                                        {{ __('task.finish_before') }}: {{ $task->deadline }}
                                                    @endif
                                                    <br>
                                                @endif
                                                <div class="mt-3">
                                                    ...
                                                    @endif
                                                    <a class="btn btn-primary btn-sm" href="{{ route('task.show', $task->id) }}">
                                                        {{ __('task.go_to_task') }}
                                                    </a>
                                                </div>
                                            </div>
                                ...
                                @endforeach
                            @else
                                {{ __('task.dont_have_active_tasks') }}
                            @endif
                        </div>
                ...
                <div class="col-md-6">
                    <div class="card">
                        <div class="card-header cursor-pointer" data-card-widget="collapse">
                            <h3 class="card-title">{{ __('task.all_income_tasks') }}</h3>
                            <div class="card-tools">
                                <button type="button" class="btn btn-tool" data-card-widget="collapse"><i class="fas fa-minus"></i></button>
                            </div>
                        </div>
                                            ...
                                            <span class="username">
                                                ...
                                                <span class="text-muted float-right">{{ $task->created_at->diffForHumans() }}</span>
                                                    ...
                                                    @if($task->deadline != '')
                                                        ...
                                                        <br>
                                                    @endif
                                                    {{ __('task.responsible') }}: {{ $task->send_to }}
                                                    @if($task->info != '')
                                                    ...
                                                    <div class="mt-3">
                                                        <button type="button"
                                                                class="btn btn-success btn-sm task_handler"
                                                                data-task_id="{{ $task->id }}"
                                                                data-action="get_task"
                                                                data-redirect_to_task="true">
                                                            <i class="fas fa-check"></i> {{ __('task.accept') }}
                                                        </button>
                                                        <div class="btn-group">
                                                            <button type="button"
                                                                    ...
                                                                    aria-expanded="false">
                                                                <i class="fas fa-share"></i> {{ __('task.redirect') }}</button>
                                                            <div class="dropdown-menu" style="">
                                                                @foreach($users as $user)
                                                                    <a class="dropdown-item cursor-pointer task_handler"
                                                                       data-action="transfer_task"
        ...
        <div class="row">
            <div class="col-md-6">
                <div class="card">
                    <div class="card-header cursor-pointer" data-card-widget="collapse">
                        <h3 class="card-title">{{ __('task.send_on_approval') }}</h3>
                        <div class="card-tools">
                            <button type="button" class="btn btn-tool" data-card-widget="collapse"><i class="fas fa-minus"></i></button>
                        </div>
                    </div>
                    <div class="card-body card-comments">
                        @if($send_for_approval->isNotEmpty())
                            @foreach($send_for_approval as $task)
                                ...
                                @endforeach
                            @else
                                {{ __('task.you_dont_have_tasks_send_to_approval') }}
                            @endif
                        </div>
                        <div class="card-footer">
