        ...
        @if(!is_null($project))
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control" name="inv_num"
                       placeholder="{{ __('invoice.invoice_number') }}" required>
            </div>
            <div class="form-group">
                <label>{{ __('invoice.invoice_date') }}</label>
                <input type="text" class="form-control date_input" name="inv_date"
                       placeholder="{{ __('invoice.invoice_date') }}" value="{{ \Carbon\Carbon::now()->format('Y-m-d') }}" required>
            </div>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" name="cont_num"
                                   placeholder="{{ __('invoice.contract_number') }}" value="{{ $cont_num }}" required>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="form-group">
                            <label for="cont_num">{{ __('invoice.contract_date') }}</label>
                            <input type="text" class="form-control date_input" name="cont_date"
                                   placeholder="{{ __('invoice.contract_date') }}" value="{{ $cont_date }}" required>
                        </div>
                    </div>
                </div>
            @endif
            <div class="form-group">
                ...
                <input type="text" class="form-control" name="client_company_name"
                       placeholder="{{ __('invoice.company_name') }}" value="{{ $client_company_name }}" required>
            </div>
            <div class="form-group">
                <label>{{ __('general.requisites') }}</label>
                <textarea class="form-control" rows="6" name="client_company_requisites"
                          placeholder="{{ __('general.requisites') }}">{{ $client_company_requisites }}</textarea>
            </div>
            <div class="form-group">
                <label>{{ __('invoice.invoice_services') }}</label>
                <textarea class="form-control" rows="4" name="invoice_services"
                          placeholder="{{ __('invoice.invoice_services') }}">{{ $invoice_services }}</textarea>
            </div>
            <div class="row">
                <div class="col-md-3">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" name="currency"
                               placeholder="{{ __('general.currency') }}" value="{{ $currency }}" required>
                    </div>
                </div>
                <div class="col-md-3">
                    <div class="form-group">
                        <label for="price_1pc">{{ __('invoice.price_1pc') }}</label>
                        <input type="text" class="form-control rate_input" name="price_1pc"
                               placeholder="{{ __('invoice.price_1pc') }}" value="{{ $price_1pc }}" required>
                    </div>
                </div>
                <div class="col-md-3">
                    <div class="form-group">
                        <label for="freight_amount">{{ __('invoice.freight_amount') }}</label>
                        <input type="text" class="form-control rate_input" name="freight_amount"
                               placeholder="{{ __('invoice.freight_amount') }}" value="{{ $freight_amount }}" required>
                    </div>
                </div>
                <div class="col-md-3">
                    <div class="form-group">
                        <label for="price_in_currency">{{ __('invoice.total') }}</label>
                        <input type="text" class="form-control" name="price_in_currency"
                               placeholder="{{ __('invoice.total') }}" value="{{ $price_in_currency }}" required>
                    </div>
                </div>
            </div>
            @else
