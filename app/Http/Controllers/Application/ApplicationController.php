...
//        }
        ...
        return view('application.show', [
            ...
            'relations' => $this->getApplicationsRelations($application)
        ]);
    }
    ...
    public function update(Request $request, Application $application)
    {
        ...
        $this->updateAutogeneratedInvoices($application);
                        ...
                        if($request->application_type == 'Клиент'){
                            if(!is_null($container->client_application_id)){
                                if($container->client_application_id != $application_id) {
                                    ...
                                    if(!is_null($container->client_date_return)){
                                        $can_reuse [] = [
...
//        }
    ...
    public function getApplicationInvoices($application){
                ...
                foreach ($terminal as $key => $value){
                    if($value != 0){
                        ...
                        $invoices [] = [
                            ...
                            'amount_in_rubles' => $in_rubles,
                            'info' => $info,
                            'autogenerated_type' => 'terminal_'.$key
                        ];
                    }
                }
                ...
                foreach ($renewal as $key => $value){
                    if($value != 0){
                        ...
                        $invoices [] = [
                            ...
                            'amount_in_rubles' => $in_rubles,
                            'info' => $info,
                            'autogenerated_type' => 'prolong_'.$key
                        ];
                    }
                }
                ...
                foreach ($repair_out as $key => $value){
                    if($value != 0){
                        ...
                        $invoices [] = [
                            ...
                            'amount_in_rubles' => $in_rubles,
                            'info' => $info,
                            'autogenerated_type' => 'repair_'.$key
                        ];
                    }
                }
        ...
        if($application->type == 'Покупка'){
            ...
            $invoices [] = [
                ...
                'autogenerated_type' => 'buy_containers'
            ];
        }
        ...
        if($application->type == 'Продажа'){
            ...
            $invoices [] = [
                ...
                'autogenerated_type' => 'sell_containers'
            ];
        }
        ...
        if(!is_null($request->all_invoices)){
            foreach (unserialize($request->all_invoices) as $key => $invoice){
                ...
                if(isset($request->$item)){
                    ...
                    $choosen_currency = $request->currency_out[$key];
                    ...
                    $new_invoice->currency = $choosen_currency;

                    if($invoice['currency'] == $choosen_currency){
                        ...
                        if($choosen_currency != 'RUB'){
                            $converted = $this->convertInvoiceCurrency($invoice['currency'], $choosen_currency, $invoice['amount_in_currency']);
                            $new_invoice->amount_in_currency = $converted['amount_in_currency'];
                            $new_invoice->rate_out_date = $converted['rate_out_date'];
                        }
                    }
                    if($invoice['type'] == 'Расход'){
                    ...
                    $new_invoice->additional_info = $this->getInvoiceAdditionalInfo($request->application_id, $invoice['info']);
                    $new_invoice->autogenerated = true;
                    $new_invoice->autogenerated_type = $invoice['autogenerated_type'];
                    $new_invoice->save();
    ...
    public function checkCanFinishApplication(Application $application){
        ...
        if(!in_array($application->status, ['Завершена', 'Черновик'])){
            if(!is_null($application->containers)){
                if($application->containers_amount == count($application->containers)) {
                    if($application->type == 'Поставщик') {
                        foreach ($application->containers as $container_name) {
    ...
    public function getApplicationsRelations(Application $application){
        $supplier_apps = [];
        $relocation_apps = [];
        $client_apps = [];
        if(!is_null($application->containers)){
        ...
        return [
            'client_applications' => empty($client_apps) ? $client_apps = null : array_unique($client_apps),
            'supplier_applications' => empty($supplier_apps) ? $supplier_apps = null : array_unique($supplier_apps),
            'relocation_applications' => empty($relocation_apps) ? $relocation_apps = null : array_unique($relocation_apps),
        ];
    }

    public function getInvoiceAdditionalInfo($application_id, $info){
        $application = Application::withTrashed()->find($application_id);

        if($info == 'Базовая ставка выдачи ктк клиенту')
            $info = 'Предоставление контейнеров в пользование по маршруту';

        $send_from = !is_null($application->send_from_city) ? implode(', ', $application->send_from_city) : '';
        $send_to = !is_null($application->place_of_delivery_city) ? implode(', ', $application->place_of_delivery_city) : '';
        ...
        !is_null($application->containers) ? $containers = implode(', ', $application->containers) : $containers = '';

        return 'Заявка №'.$application->name.' от '.$application->created_at->format('d.m.Y').'г. '.$info.' '.$send_from.' - '.$send_to.', '.$application->containers_amount.'*'.$application->containers_type.', '.$application->price_amount.$application->price_currency.'/'.$application->grace_period.'/'.$snp.', Итого '.$application->price_amount*$application->containers_amount.$application->price_currency.'. '.$containers;
    }

    public function convertInvoiceCurrency($from, $to, $amount_in_currency){
        $currency_rates = CurrencyRate::orderBy('created_at', 'desc')->first();

        if($from == 'USD' && $to == 'CNY'){
            $amount_in_currency = round($currency_rates['USD'] / $currency_rates['CNY'] * $amount_in_currency, 2);
            $rate_out_date = $currency_rates['CNY'];
        }
        elseif($from == 'CNY' && $to == 'USD'){
            $amount_in_currency = round($currency_rates['CNY'] / $currency_rates['USD'] * $amount_in_currency, 2);
            $rate_out_date = $currency_rates['USD'];
        }
        else {
            $amount_in_currency = round($currency_rates[$to] * $amount_in_currency, 2);
            $rate_out_date = $currency_rates[$to];
        }

        return [
            'amount_in_currency' => $amount_in_currency,
            'rate_out_date' => $rate_out_date
        ];
    }

    public function updateAutogeneratedInvoices(Application $application){
        if($application->invoices->count() != 0){
            $currency_rates = CurrencyRate::orderBy('created_at', 'desc')->first();
            $updated_invoices = $this->getApplicationInvoices($application);
            foreach ($application->invoices as $invoice){
                if(!in_array($invoice->status, ['Оплачен', 'Частично оплачен']) && $invoice->autogenerated == 1){
                    foreach ($updated_invoices as $updated_invoice){
                        if($updated_invoice['autogenerated_type'] == $invoice->autogenerated_type){
                            if($updated_invoice['amount_in_rubles'] != $invoice->amount){
                                if($updated_invoice['currency'] != $invoice->currency){
                                    $converted = $this->convertInvoiceCurrency($updated_invoice['currency'], $invoice->currency, $updated_invoice['amount_in_currency']);
                                    $invoice->update([
                                        'amount' => $updated_invoice['amount_in_rubles'],
                                        'amount_in_currency' => $converted['amount_in_currency'],
                                        'additional_info' => $this->getInvoiceAdditionalInfo($application->id, $updated_invoice['info']),
                                        'rate_out_date' => $converted['rate_out_date']
                                    ]);
                                }
                                else {
                                    $invoice->update([
                                        'amount' => $updated_invoice['amount_in_rubles'],
                                        'amount_in_currency' => $updated_invoice['amount_in_currency'],
                                        'additional_info' => $this->getInvoiceAdditionalInfo($application->id, $updated_invoice['info']),
                                        'rate_out_date' => $currency_rates[$invoice->currency]
                                    ]);
                                }
                            }
                        }
    ...
    public function changeStatusInWork(Request $request){
        $application = Application::withTrashed()->find($request->application_id);
        $application->update([
            'status' => 'В работе'
        ]);

        return redirect()->back()->withSuccess('Статус был успешно изменен');
    }
}
