...
class ReportController extends Controller
{
    ...
    public function incomeByTypes(Request $request){
        ...
        $income_types = IncomeType::whereNotNull('category')->orderBy('category', 'ASC')->get()->toArray();

        $output_array = [];

        $i = 0;

        foreach ($income_types as $key => $type){
            $output_array[$i]['type'] = $type['name'];
            $output_array[$i]['category'] = $type['category'];
            $output_array[$i]['type_total'] = 0;
            $output_array[$i]['month_1'] = 0;
            ...
            if(array_key_last($income_types) > $key){
                if($income_types[$key+1]['category'] != $output_array[$i]['category']){
                    $i++;
                    $output_array[$i]['type'] = 'ИТОГО';
                    $output_array[$i]['category'] = $type['category'];
                    $output_array[$i]['type_total'] = 0;
        ...
        $spreadsheet = \PhpOffice\PhpSpreadsheet\IOFactory::load("storage/templates/incomes_by_types_template.xlsx");
        $sheet = $spreadsheet->getActiveSheet();
        ...
        $writer = \PhpOffice\PhpSpreadsheet\IOFactory::createWriter($spreadsheet, "Xlsx");
        $path = 'storage/Отчеты/'.config('app.prefix_view').'сводка_по_доходам_'.date('dmY').'.xlsx';
        $writer->save($path);

        $download_path = 'public/Отчеты/'.config('app.prefix_view').'сводка_по_доходам_'.date('dmY').'.xlsx';
        return Storage::download($download_path);
